...
type PartnerItemProps = {
  index: number;
  updateImg: (e: string, name: string, index: number, id?: string | undefined) => void;

  handleRemoveByIndex: (deletedIndex: number) => void;
} & Partial<DynamicContent>;

const PartnerItem = ({ index, content, contentEn, updateImg, handleRemoveByIndex, id }: PartnerItemProps) => {
  const { trigger: removePartner, isMutating } = useSWRMutation(id && dynamicContentApis.delete(id), getData);
